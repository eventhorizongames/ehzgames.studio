...
public class GenerateOutput : IClassFixture<AppTestFixture>
{
    ...
    {
        return PrerenderRouteHelper
            .GetRoutes(typeof(Website.App).Assembly) // Pass in the WebAssembly app's Assembly
    ...
    public async Task Render(string route)
    {
        ...
        using (
            var compressedFileStream = new FileStream(
                filePath + ".gz",
                FileMode.Create,
                FileAccess.Write,
                FileShare.None
        ...
        using var brotliCompressedFileStream = new FileStream(
            filePath + ".br",
            FileMode.Create,
            FileAccess.Write,
            FileShare.None
